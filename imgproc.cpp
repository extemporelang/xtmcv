...
Contour ApproxPolyDP(Contour curve, double epsilon, bool closed) {
    ...
    for (size_t i = 0; i < length; i++) {
        // changed by AS
        Point p;
        p.x = approxCurvePts[i].x;
        p.y = approxCurvePts[i].y;
        points[i] = p;
    ...
    c.points = points;
    c.length = length;
    return c;  //(Contour){points, length};
}
...
struct Size* GetTextSize(const char* text, int fontFace, double fontScale, int thickness) {
    ...
    Size* size = toSize(sz.width, sz.height);
    return size;
}
