...
Mat Mat_NewFromByteArray(int rows, int cols, int type, struct ByteArray* buf) {
    return new cv::Mat(rows, cols, type, buf->data);
}

Mat Mat_NewFromBytes(int rows, int cols, int type, char* buf) {
...
void Mat_DataPtr(Mat m, char** outData, int* outSize) {
    *outData = reinterpret_cast<char *>(m->data);
    *outSize = m->total() * m->elemSize();
    return;
}

// struct char* Mat_DataPtr(Mat m) {
...
char* ByteArray_data(struct ByteArray* s) { return s->data;  }
int ByteArray_length(struct ByteArray *s) { return s->length; }
...
void IntVector_Release(struct IntVector* s) {
    delete[] s->val;
    delete s;
}

struct IntVector* toIntVector(const int* buf, int len) {
    IntVector *ret = new IntVector();
    ret->val = new int[len];
    ret->length = len;
    memcpy(ret->val, buf, len);
    return ret;
}

void FloatVector_Release(struct FloatVector* s) {
    delete[] s->val;
    delete s;
}

struct FloatVector* toFloatVector(const float* buf, int len) {
    FloatVector *ret = new FloatVector();
    ret->val = new float[len];
    ret->length = len;
    memcpy(ret->val, buf, len);
    return ret;
}

void Size_Release(struct Size* s) {
    delete s;
}

struct Size* toSize(int width, int height) {
    Size *ret = new Size();
    ret->width = width;
    ret->height = height;
    return ret;
...
int Point_x(struct Point *s) { return s->x; }
int Point_y(struct Point *s) { return s->y; }

void Point_Release(struct Point* s) {
    delete s;
}
...
void Points_Release(struct Points* s) {
    delete[] s->points;
    delete s;
}

struct Points* toPoints(struct Point* buf, int len) {
    Points *ret = new Points();
    ret->points = new Point[len];
    ret->length = len;
    memcpy(ret->points, buf, len);
    return ret;
}

void DMatch_Release(struct DMatch* s) {
    delete s;
}

struct DMatch* toDMatch(int queryIdx, int trainIdx, int imgIdx, float distance) {
    DMatch *ret = new DMatch();
    ret->queryIdx = queryIdx;
...
void DMatches_Release(struct DMatches* s) {
    delete[] s->dmatches;
    delete s;
}

struct DMatches* toDMatches(struct DMatch* buf, int len) {
    DMatches *ret = new DMatches();
    ret->dmatches = new DMatch[len];
    ret->length = len;
    memcpy(ret->dmatches, buf, len);
    return ret;
}

void KeyPoint_Release(struct KeyPoint* s) {
    delete s;
}

struct KeyPoint* toKeyPoint(double x, double y, double size, double angle, double response, int octave, int classID) {
    KeyPoint *ret = new KeyPoint();
    ret->x = x;
    ret->y = y;
    ret->size = size;
    ret->angle = angle;
...
void KeyPoints_Release(struct KeyPoints* s) {
    delete[] s->keypoints;
    delete s;
}

struct KeyPoints* toKeyPoints(struct KeyPoint* buf, int len) {
    KeyPoints *ret = new KeyPoints();
    ret->keypoints = new KeyPoint[len];
    ret->length = len;
    memcpy(ret->keypoints, buf, len);
    return ret;
}

void Contours_Release(struct Contours* s) {
    delete[] s->contours;
    delete s;
}

struct Contours* toContours(struct Points* buf, int len) {
    Contours *ret = new Contours();
    ret->contours = new Contour[len];
    ret->length = len;
    memcpy(ret->contours, buf, len);
    return ret;
}

void Scalar_Release(struct Scalar* s) {
    delete s;
}

struct Scalar* toScalar(double val1, double val2, double val3, double val4) {
...
void Rect_Release(struct Rect* s) {
    delete s;
}

struct Rect* toRect(int x, int y, int width, int height) {
    Rect *ret = new Rect();
    ret->x = x;
    ret->y = y;
    ret->width = width;
...
void Rects_Release(struct Rects* s) {
    delete[] s->rects;
    delete s;
}

struct Rects* toRects(struct Rect* buf, int len) {
    Rects *ret = new Rects();
    ret->rects = new Rect[len];
    ret->length = len;
    memcpy(ret->rects, buf, len);
    return ret;
}
