...
char* ByteArray_data(struct ByteArray *s);
int ByteArray_length(struct ByteArray *s);
...
int Point_x(struct Point *s);
int Point_y(struct Point *s);
...
void IntVector_Release(struct IntVector* vec);

struct FloatVector* toFloatVector(const float* buf, int len);
void FloatVector_Release(struct FloatVector* vec);

struct Rect* toRect(int x, int y, int w, int h);
void Rect_Release(struct Rect* s);

struct Rects* toRects(struct Rect*, int len);
...
struct Points* toPoints(struct Point*, int len);
...
struct KeyPoints* toKeyPoints(struct KeyPoint*, int len);
...
struct DMatches* toDMatches(struct DMatch*, int len);
void DMatches_Release(struct DMatches* s);

struct Contours* toContours(struct Points*, int len);
void Contours_Release(struct Contours* s);

struct Scalar* toScalar(double v1, double v2, double v3, double v4);
